...
import static util.SkillKeys.SHARE_FART_INTENT;
...
public class ShareFartIntentHandler implements IntentRequestHandler {
    @Override
    public boolean canHandle(HandlerInput handlerInput, IntentRequest intentRequest) {
        return intentRequest.getIntent().getName().equals(SHARE_FART_INTENT);
    }
        ...
        boolean shouldEndSession = false;
         ...
         if (intentRequest.getIntent().getSlots() != null
            ...
            if(!dbHelper.userExists(targetUserName)){
                speechText = "FARTING, <audio src='https://d383tqkx4loq8a.cloudfront.net/test-response.mp3' />";
            } else{
                ...
                dbHelper.saveMessageToDDB(currentSessionAttributes.get(USER_NAME_SESSION_KEY).toString(), targetUserName,
                        "FARTING, <audio src='https://d383tqkx4loq8a.cloudfront.net/test-response.mp3' />");
            }
